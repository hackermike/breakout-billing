...
A superbill is the itemized "Statement for Insurance Reimbursement" a client
submits to their insurer for out-of-network reimbursement. It carries the
provider's NPI and credentials, the client's details, and per-session CPT/ICD-10
codes with fees. Operates on the `domain.py` Protocols — no ORM dependency.
"""
...
from breakout_core.domain import AppointmentLike, ClientLike, ProviderLike
from breakout_core.finances import appt_paid
...
def payments_to(client: ClientLike) -> str:
    """Whom the client remits payment to (their own name, for reimbursement)."""
    return f"{client.first_name} {client.last_name}".strip()


def service_rows(appointments: list[AppointmentLike], client: ClientLike) -> list[dict]:
...
def build_superbill_pdf(
    ...
    client: ClientLike,
    appointments: list[AppointmentLike],
    start: date,
    end: date,
) -> bytes:
    pdf = FPDF()
